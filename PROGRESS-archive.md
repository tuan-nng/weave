...
### feat-054 — SessionPage runtime/mode layout switcher (impl 2026-06-11)

Phase 9, second feature. The session page (`/sessions/:id`) now renders one of three layouts from the same URL, chosen by the session's `mode` column. `native` layout: existing chat chrome unchanged (HTTP-backed sessions). `wrapped` layout: same chat view body, but `SessionHeader` gains a pill row showing the runtime name, the active permission mode, and the per-turn resume state; a small `WrappedSessionBanner` surfaces the new interaction model on the first turn. `attended` layout: reserved for Phase 11, defensively renders as native. The session was opened via `/feature-dev:feature-dev start next task` and ran the full 7-phase workflow.

**Phase 1-3** (Discovery, Exploration, Clarifying Questions): confirmed next feature is feat-054; mapped the existing `pages/session.tsx` (inline header at lines 856-898, 994-line page) and the SSE handler at `use-session.ts:368-503`. Discovered a **CRITICAL gap** that became the prerequisite: the frontend `Session` interface (web/src/lib/types.ts:14-30) was MISSING `runtime_kind`, `mode`, and `runtime_metadata_json` even though the backend serializes them. Same gap on `SseDoneEvent` and `SseMessagePersistedEvent` — they lacked the per-turn `runtime_kind` / `mode` / `resume_state` fields the backend already broadcasts. Resolution: widen the frontend types as part of feat-054 (the spec's "layout switcher from `session.mode`" is impossible without the type widening). Resolved 5 user clarifications:
- **Layout switcher (Q1)**: single component with conditional header/banner (recommended) — no separate `WrappedSessionPage` / `NativeSessionPage`.
- **Banner copy (Q2)**: subprocess callout, dismissable, persists across page loads (recommended) — localStorage key `weave.dismissed.wrappedSessionBanner.{sessionId}`.
- **Resume state pill (Q3)**: color-coded 3-state pill, always shown (recommended) — `none` slate, `native` emerald, `replayed` amber.
- **Display name source (Q4)**: client-side `useProviders()` lookup of `Provider.name` by `session.provider_id` (recommended) — the backend has no per-session `display_name` field; the `Provider.name` row is the source of truth.
- **Permission mode source (Q5, surfaced during Phase 4)**: use the static `Provider.permission_mode` column value, NOT a per-turn `PermissionSnapshot` — the snapshot table doesn't exist yet; the static value matches the spec's intent until a future feature wires snapshots.

**Phase 4** (Architecture Design): single architect with 2-3 tradeoff lenses (small/medium scope, 1-3 files). User approved the recommended design:
- Type widening in `lib/types.ts` (`RuntimeKind` 6-value union, `SessionMode` 3-value union, new `ResumeState` 3-value union; widen `Session`, `SseDoneEvent`, `SseMessagePersistedEvent`; widen `CreateSessionRequest.runtime_kind` to the new union).
- `LiveBuffer.lastResumeState: ResumeState | null` (last-wins via `MESSAGE_PERSISTED` + `DONE` reducer cases; `SEND_STARTED` resets to `null`).
- New `components/session-header.tsx` (extracted header + wrapped pill row).
- New `components/wrapped-session-banner.tsx` (first-turn callout, localStorage-dismissable).
- `pages/session.tsx` swaps inline header → `<SessionHeader>`; adds `<WrappedSessionBanner>` above the message list (gated on `session.mode === "wrapped"` and `firstTurn`); adds `useProviders()` lookup of provider by id.
- 0 backend changes (the wire shape was already correct from feat-038 / feat-047).
- New test file `app/__tests__/session-layouts.test.tsx` (17 tests covering SessionHeader, WrappedSessionBanner, reducer transitions, defensive attended fallback).

**Phase 5** (Implementation): build sequence followed the architect's blueprint:
1. Widen `web/src/lib/types.ts`: add `RuntimeKind` (6 values), `SessionMode` (3 values), `ResumeState` (3 values) as string-literal unions; add 3 fields to `Session`; add 3 fields to `SseDoneEvent` and `SseMessagePersistedEvent`; widen `CreateSessionRequest.runtime_kind` to the new `RuntimeKind` union.
2. Update `web/src/lib/runtime-matrix.ts`: re-export `RuntimeKind` / `SessionMode` from `types.ts` (was defined as `NonNullable<CreateSessionRequest["runtime_kind"]>` here); widen `SPECIALIST_PROFILE_COMPAT` to cover all 6 runtime kinds (`"openai-api"` and `"openai-compatible"` added).
3. Update `web/src/hooks/use-session.ts`: add `lastResumeState: ResumeState | null` to `LiveBuffer`; update `EMPTY_LIVE_BUFFER`; widen `MESSAGE_PERSISTED` and `DONE` action variants; reducer cases fold `resumeState` (last-wins); SSE handlers forward the new fields.
4. Create `web/src/components/session-header.tsx` (NEW, 159 lines): extracted header with a `Pill` helper, `RUNTIME_KIND_LABEL` fallback table, and `RESUME_STATE_PILL` color map. Defensive attended fallback to native (no pill row).
5. Create `web/src/components/wrapped-session-banner.tsx` (NEW, 122 lines): first-turn callout, dismissable, localStorage-persisted per session id; `useState` initializer reads once, `useEffect` listens to `storage` events for cross-tab sync; localStorage write failure is caught silently (in-memory `dismissed` flag is the fallback).
6. Update `web/src/app/pages/session.tsx`: import `useProviders` + new components, remove `Link` + `StatusBadge` imports (now used in `SessionHeader`); add `useMemo` to resolve provider by id from the providers list; replace the 43-line inline header with `<SessionHeader>`; add `<WrappedSessionBanner>` above the message list (gated on `session.mode === "wrapped"` and `firstTurn = messages.filter(role === "assistant").length === 0 && liveBuffer.persistedTurnId === null`).
7. Create `web/src/app/__tests__/session-layouts.test.tsx` (NEW, 17 tests): SessionHeader native (no pill row), wrapped (runtime/permission/resume pills, provider-null fallback, attended defensive fallback); WrappedSessionBanner (renders, dismisses, hides on same render, scopes by session id, hidden when `firstTurn: false`); reducer (`MESSAGE_PERSISTED` + `DONE` + `SEND_STARTED` reset + `CANCEL_OPTIMISTIC` preservation).
8. Prettier auto-fix on the 3 new files (whitespace cleanup).
9. `simplify` pass: confirmed all extracted components are minimal; the `Pill` helper is shared by the 3 runtime pills (worth keeping); the runtime label map and resume state pill map are typed as `Record<>` to enforce exhaustiveness at compile time.

**Phase 6** (Quality Review): 1 `code-reviewer` agent. **0 issues ≥ 80 confidence.** Verdict: passes. The 9 user-specified concerns were investigated and verified clean: (1) no other frontend file constructs a typed `Session` literal that misses the new fields; (2) no test or production code constructs an `SseMessagePersistedEvent` / `SseDoneEvent` literal; (3) `useProviders()` + `useMemo` deps `[session, providersQuery.data]` correctly handle cross-session navigation; (4) banner `firstTurn` is hidden when an assistant message is persisted — matches the "first turn callout" spec; (5) `setDismissed(true)` runs before the localStorage `try/catch` so the in-memory flag is the right fallback; (6) `RUNTIME_KIND_LABEL` covers all 6 runtime kinds and is `Record<>`-typed; (7) `SEND_STARTED` resets `lastResumeState` via `...EMPTY_LIVE_BUFFER` spread; (8) the 17 unit/component tests are the right granularity (page-level integration is not directly tested — accepted limitation, same pattern as the existing `session-traces.test.tsx`); (9) the `as SseDoneEvent` / `as SseMessagePersistedEvent` casts have a falsy guard (`resumeState &&` at `session-header.tsx:127`) that catches `undefined` from a hypothetical older backend.

**Phase 7** (Summary): this entry. Updated `feature_list.json:798` to `state: "passing"` with the detailed evidence paragraph. Updated `PROGRESS.md` (current state, next steps, OOS items). The `not_started` → `passing` state transition flows through `feature_list.json` per CLAUDE.md rule 10.

**Files modified this session (final list, ordered by build dependency):**

1. `web/src/lib/types.ts` — 3 new type aliases + 3 fields on `Session` + 3 fields on each of 2 SSE events + `CreateSessionRequest.runtime_kind` widened to new union (+65 lines, -3).
2. `web/src/lib/runtime-matrix.ts` — re-export `RuntimeKind` / `SessionMode` from `types.ts` (was defined here); widen `SPECIALIST_PROFILE_COMPAT` (+2 entries) (+8 lines, -8).
3. `web/src/hooks/use-session.ts` — `LiveBuffer.lastResumeState` + `EMPTY_LIVE_BUFFER` update + widened `MESSAGE_PERSISTED` / `DONE` actions + reducer cases + SSE handler forwarding (+30 lines, -3).
4. `web/src/components/session-header.tsx` (NEW, 159 lines) — extracted header + wrapped pill row.
5. `web/src/components/wrapped-session-banner.tsx` (NEW, 122 lines) — first-turn callout, localStorage-persisted.
6. `web/src/app/pages/session.tsx` — removed inline header, added `useProviders()` + `useMemo`, added `<WrappedSessionBanner>`, swapped imports (+44 lines, -47).
7. `web/src/app/__tests__/session-layouts.test.tsx` (NEW, 246 lines) — 17 tests covering all of the above.
8. `feature_list.json` — `feat-054` flipped to `passing` with full evidence paragraph.
9. `PROGRESS.md` — current state, next steps, OOS items.
10. `PROGRESS-archive.md` — this entry.

**Verification baseline (commit before this session was `8ccc684`; the commit after will be the feat-054 commit):**

- `./init.sh` exit 0, all 3 layers pass.
- 791 Rust tests + 141 frontend tests, 17 new in feat-054.
- clippy clean, fmt clean, prettier clean, ESLint clean.
- Server starts, `/api/health` 200, `GET /` serves index.html with `id="root"`, graceful shutdown.

**Key decisions made this session:**

- **Type widening prerequisite (Phase 3 discovery)**: the frontend `Session` interface was missing the 3 fields the backend serializes (this is a Phase 9 carry-over — feat-038 added the columns and feat-053 widened `CreateSessionRequest`, but no one widened the read-side `Session` type). The cleanest fix was to widen as part of feat-054 rather than a separate "pre-054" cleanup, because the layout switcher itself is impossible without `session.mode`. Same call applies to `SseDoneEvent` / `SseMessagePersistedEvent` (the wire shape was correct on the server, missing on the client).
- **Single component, conditional header/banner (Phase 4)**: the spec called for "isolated to `SessionHeader` and `WrappedSessionBanner`" with 95% component overlap. Two new files + the page's switch-via-`session.mode` is the smallest diff that delivers the spec. A separate `<NativeSessionPage>` vs `<WrappedSessionPage>` would have been a larger refactor with no runtime benefit.
- **Provider lookup via `useProviders()` + `find()` (Phase 4)**: the alternatives considered were (a) a new `useProvider(id)` hook that fetches a single provider, and (b) a `display_name` field on the session row. (a) adds a network round-trip per session load. (b) duplicates data the operator already entered on the provider row. The chosen approach re-uses the existing `useProviders()` global query (which `NewSessionWizard` already populates for its Step 0 list) and resolves by id in a `useMemo` over `[session, providersQuery.data]`. 0 new endpoints, 0 new fields, 0 new network calls in the warm-cache path.
- **Banner `firstTurn` is a derived prop (Phase 4)**: making the banner a presentational component with `firstTurn` as a prop keeps the "have we seen an assistant message yet" question in the page, where the persisted history and the live buffer are both available. The alternative — having the banner subscribe to the history query itself — would have duplicated the data path and made the dismissal semantics unclear.
- **`RuntimeKind` / `SessionMode` moved to `lib/types.ts` (Phase 5)**: prior definitions in `runtime-matrix.ts` were `NonNullable<CreateSessionRequest["runtime_kind"]>`. Moving the canonical definitions to `types.ts` lets `SseDoneEvent` and `SseMessagePersistedEvent` carry the same string-literal unions. The re-export from `runtime-matrix.ts` keeps the wizard's call sites unchanged. The cost: one new cross-file type source for two new callers (the SSE event types); the benefit: a single source of truth for the wire form (the canonical `kebab-case` / `snake_case` comments live in one place).
- **No `Pill` component reuse beyond the header (Phase 5 simplify)**: the spec's "small pill row" wording suggested 3 reusable pills. The 3 pills have different shapes (runtime/permission are static, resume state is per-turn), so a `Pill` helper is shared but no `ResumeStatePill` / `RuntimePill` components were extracted. The single `Pill({ label, className, title })` helper is the right granularity — the alternative would have been 3 named exports with one line of differentiation between them.
- **Attended defensive fallback (Phase 5)**: `session.mode === "attended"` renders as native (no pill row, no banner). The server rejects attended at create time, so the only way a row lands with `mode: "attended"` is a database row written before feat-054's create-time validation, which is impossible today. The defensive `const isWrapped = session.mode === "wrapped"` makes the contract explicit and survives any future "attended" surface changes.
- **localStorage write failure is silent (Phase 5)**: the banner's `handleDismiss` flips the in-memory `setDismissed(true)` first, then attempts the `localStorage.setItem` inside a `try/catch` that swallows errors. The reasoning: localStorage can fail in private-browsing mode (Safari) or under quota pressure; the user-visible contract is "the banner goes away on click", which is delivered by the in-memory flag regardless of storage availability.
- **Resume-state pill is "always shown when set" (Phase 5)**: the pill renders whenever `resumeState !== null`. Before the first event of a turn (or for HTTP runtimes that never have a resume id), the pill is hidden — the absence of a pill IS the signal that the runtime hasn't reported a resume outcome yet. This matches the spec's "per-turn resume state" wording.
- **`LiveBuffer.lastResumeState` is last-wins, not append-only (Phase 5)**: both `MESSAGE_PERSISTED` and `DONE` fold `resumeState` into the live buffer; both events emit the same value per turn (the server's `ResumeState::decide` is called once per turn and broadcast on both). The `SEND_STARTED` case resets to `null` so a new turn's pill doesn't show the previous turn's color before the first event lands.
- **`useProviders` lookup is `data?.data.find()` → `data?.find()` (Phase 5 typecheck fix)**: the `api.providers.list` endpoint returns `Provider[]` directly (not wrapped in an envelope), so the first draft's `providersQuery.data.data.find()` was a type error. Fixed to `providersQuery.data.find(p => p.id === session.provider_id)`. The `apiFetch<T>` helper is envelope-agnostic — some endpoints return envelopes, some don't.
- **`SPECIALIST_PROFILE_COMPAT` widened for type completeness (Phase 5)**: the `Record<RuntimeKind, ReadonlySet<string>>` exhaustive-map type gained 2 entries (`"openai-api"` and `"openai-compatible"`) so the new `RuntimeKind` union's exhaustiveness check stays green. The runtime matrix remains "all profiles accepted" everywhere (each Set is empty), so the behavior is unchanged — the type widening is purely a `Record<>` completeness fix.

**Cross-feature follow-ups (now in PROGRESS.md OOS):**

- feat-055 (kanban column `(runtime_kind, specialist_id)` binding) — independent of feat-054, but the frontend `RuntimeKind` union is now the source of truth for both the wizard (Step 0) and the session header (pill row). When feat-055 adds the `ColumnHeader` Runtime Tool badge, the same `RUNTIME_KIND_LABEL` fallback table is the natural home for the badge label.
- feat-057 (conformance suite) — should add `FromStr` to `ResumeState` (mirroring `SessionMode::from_str`) when it needs to parse the persisted string form. The frontend's `ResumeState` union is now in place; the backend parser is the missing piece.
- feat-058/059 (Codex/OpenCode) — the `lastResumeState` field on `LiveBuffer` is a generic shape; the same wiring (reducer folds `resumeState` from the SSE event) works for any CLI runtime. The `RUNTIME_KIND_LABEL` map already has entries for `codex` and `opencode`.
- Phase 11 (attended) — when the attended layout is implemented, the `SessionHeader` already treats `mode === "attended"` as native. The new layout's chrome (e.g. a "session is in attended mode — go to the CLI to drive" callout) can be added by widening the `showPills` derivation to `mode === "wrapped" || mode === "attended"`. The `WrappedSessionBanner` is wrapped-specific and stays as-is.
- Backend `FromStr` for `ResumeState` (was OOS in feat-047) — feat-054's frontend `ResumeState` union is the natural forcing function. When the conformance suite (feat-057) lands, the `FromStr` impl is a 5-line add mirroring `SessionMode::from_str`.

---

### fix-071 — New-session wizard Step 1 reports "No healthy providers" on a cold cache (bug fix, 2026-06-12)

**Symptom (from the user):** on a fresh server boot, opening `http://localhost:5173/sessions` and clicking `+ New Session in default` rendered the wizard's Step 1 of 4 (Runtime) with `No healthy providers. Visit Settings to configure a provider.` — even though the user's `claude-code` CLI provider (`/home/novpla/.local/bin/claude`) was in the DB and `claude --version` actually succeeded. A `GET /api/providers` against the same backend returned both rows with `healthy: false`. The wizard's `Next` button was permanently disabled.

**Root cause:** `list_providers` (`crates/weave-server/src/api/providers.rs:51-58`, pre-fix) read per-id health via the sync `cached_health_for` accessor. The accessor returns `false` for any id absent from the `HealthCache` snapshot — and the snapshot is only populated by `cached_health_summary`, which the *only* existing caller (`/api/health` in `crates/weave-server/src/api/health.rs:72`) was triggering. Nothing on the frontend's hot path calls `/api/health` (`api.health` exists in `web/src/lib/api.ts:109` but is referenced only from a test, never from a real page). So on first page load after server boot (or after `add_agent` / `remove_agent` invalidated the cache), the cache was cold and every provider surfaced as `healthy: false`.

**Affected invariant:** the code assumed some code path kept the cache warm. The docstring on `cached_health_for` even named that path explicitly — `/api/health` — but no page on the frontend actually hits it. The behavior was self-consistent (`healthy: false` is the literal "unseen" state) and self-perpetuating (no caller could warm the cache without also surfacing a 200 from `/api/health`, which the wizard never made).

**Why CI didn't catch it:** the existing `test_provider_healthy_field_default_false_when_cache_cold` enshrined the buggy behavior as a feature — its docstring said "cold-start safety" and asserted the false. The other test (`test_provider_healthy_field_reflects_cache_after_probe`) proved the warmed path, but the warm path is only reachable by calling `/api/health` first. The wire contract (the frontend trusts `healthy: true|false` to drive the wizard) was never tested end-to-end.

**Fix (one line in `api/providers.rs` + 2 tests):** call `state.registry.cached_health_summary().await` at the top of `list_providers`. `cached_health_summary` is the cache-aware accessor: on warm cache it returns instantly from the 10s TTL; on cold cache it probes all registered agents in parallel and populates the snapshot. After the call, `cached_health_for` reads a fresh snapshot. No public API change; `cached_health_for` stays a pure sync read for its other callers (A2A `messages.rs:240,264` — they have a different blast radius and get the same treatment as a follow-up).

**Regression tests (both deterministic, no real network):**
- `test_list_providers_warms_cache_on_cold_start` (RED → GREEN) — registers a stub `CodingAgent` that returns `healthy: true` directly into the registry, then calls `GET /api/providers` *without* a prior `/api/health` warm-up. Asserts the response surfaces `healthy: true`. This is the user-visible bug: a CLI provider with a working binary on a cold cache.
- `test_list_providers_reflects_failed_probe_as_unhealthy` — replaces the misleading `test_provider_healthy_field_default_false_when_cache_cold`. Same shape, but uses a stub unhealthy agent instead of a real HTTP provider, so the test is independent of `https://api.anthropic.com` reachability. The `test_provider_healthy_field_reflects_cache_after_probe` test is unchanged and still passes.

**Doc updates:** the `list_providers` doc comment is widened with a `**fix-071:**` paragraph explaining the warm-up, why the docstring previously pointed at `/api/health`, and the cache behavior (cold → probe-all in parallel; warm → instant). The pre-existing docstring on `cached_health_for` still says "typically from the `/api/health` aggregate endpoint" — that's now secondary copy, the warming happens wherever `list_providers` is hit. Not worth re-writing; future readers will see the `list_providers` fix-071 paragraph first.

**Why a minimal one-line change was right:** the design intent of `cached_health_for` (a sync pure read of the snapshot) is correct for callers that need a lock-only check (A2A message dispatch, the wizard's per-row decision). The bug was that `list_providers` — the one place that *knows* the cache might be cold on first hit — didn't trigger a warm-up. The fix preserves the sync-read API and only changes the one endpoint whose behavior was wrong. The alternative — making `cached_health_for` `async` and self-warming — would have rippled into A2A, all tests, and the `&self` → `async` signature churn, for the same end result.

**Verification:**

- `cargo test -p weave-server --lib -- api::providers::` → 24/24 pass (was 22; +2 regression tests, -1 misleading test).
- `cargo test -p weave-server` → 824/824 pass (was 804; +20 includes 18 pre-existing conformance tests + 2 new from this fix).
- `cd web && bun run test` → 142/142 pass — wizard and settings tests untouched.
- `cargo fmt --check` clean; `cargo clippy --all-targets` reports 0 new warnings (37 pre-existing in `service/{kanban,sessions}.rs` and `cli_conformance.rs`, all from earlier work and not feat-related).
- `./init.sh` → all 3 layers pass; server starts, real `claude --version` health-check on `/home/novpla/.local/bin/claude` succeeds in ~80ms (`outcome=success(0)` in the log), `/api/health` 200, `GET /` serves `index.html`.
- Live API reproduction (curl against the running server, port 3000, with the live DB that has `mm` HTTP and `claude` CLI providers):
  - Before fix: `GET /api/providers` → both rows `healthy: false`. Step 1 of the wizard shows "No healthy providers" with `Next` permanently disabled.
  - After fix: `GET /api/providers` (no prior `/api/health`) → both rows `healthy: true`. Step 1 of the wizard shows both as selectable options; picking `claude` enables `Next`; the wizard advances to Step 2 (Role).
- Diff is 1 file (`crates/weave-server/src/api/providers.rs`), +130 / -16 (mostly the 2 new tests, the fix-071 doc-comment block, and a one-line addition in `list_providers`).
