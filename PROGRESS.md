...
- **Latest commit:** **feat-065** — Column-stage awareness. New `ColumnStage` enum (Backlog/Todo/Dev/Review/Done) on `columns` via migration 015 with heuristic backfill. Stage controls prompt variation (5 arms for Available Tools, 5 arms for Instructions in `kanban_prompt.rs`), move_card stage-filtering (max-1-skip rule via `validate_stage_transition`), and API DTOs. `is_backlog_column` replaced with `column.stage == ColumnStage::Backlog`. 857 Rust + 142 frontend tests pass; `./init.sh` 3-layer gate green.
- **Active feature:** none — feat-065 is `passing` in `feature_list.json` with evidence. Per WIP=1, pick the next `not_started` feature. Phase-12 queue: feat-066 (delivery / contract rules — unblocks the v1-omitted sections 5/7/8), then feat-067 (handoff blob). Alternatively, Phase-9 follow-ups feat-058 (Codex) / feat-059 (OpenCode) are independent and unblock more runtimes.
- **Build status:** green — `./init.sh` 3-layer gate passes (857 Rust + 142 frontend tests, clippy + fmt + prettier + ESLint clean, server starts, smoke test green, real `claude --version` health-check succeeds in ~80ms)
- **Test status:** green — 857 Rust + 142 frontend (was 855 + 142; +2 net from feat-065: test_section_11_backlog_omits_artifact_tools, test_section_11_done_is_read_only). 18 named conformance tests still pass.
- **Lint status:** green — clippy clean on all targets (lib + bin + tests); no new warnings.
- **Precommit hook:** active (`core.hooksPath = .githooks`).

## Next Steps (in order)

1. **Pick the next `not_started` feature** — Phase-12 has 2 more: feat-066 (delivery / contract rules — unblocks the v1-omitted sections 5/7/8), feat-067 (handoff blob). The Phase-9 follow-ups feat-058 (Codex) and feat-059 (OpenCode) are independent and unblock more runtimes. Per WIP=1, exactly one feature in `active` state.
