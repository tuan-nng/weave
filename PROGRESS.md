...
- **Latest commit:** **feat-067** — Lifecycle supervision for kanban-auto-spawned sessions. New `kanban_session_watch` table (migration 017, `session_id PK FK CASCADE, task_id, last_activity_at, recovery_count, status`) + `store/kanban_session_watch.rs` CRUD (11 tests). `SseManager` gains `with_db(Arc<Db>)` constructor + `SseWireEvent::SessionFailed` variant; `broadcast` hook fires a fire-and-forget `bump_activity` (no-op on non-watching rows — regression guard for the un-fail race). New `service/kanban_lifecycle.rs` with 30s-interval `start` (mirrors `reap_orphans`), `scan_once`, `recover_stalled`, `build_recovery_prompt` (1KB char-boundary truncation). `SessionFailed` broadcasts on BOTH session channel and board channel. `try_automate_lane` writes the initial watch row; `send_prompt` bumps activity. 891 Rust + 142 frontend tests pass; `./init.sh` 3-layer gate green; supervisor logs `started` / `shutdown received, exiting` cleanly.
- **Active feature:** none — feat-067 is `passing` in `feature_list.json` with evidence. Per WIP=1, pick the next `not_started` feature. Phase-10 follow-ups feat-058 (Codex) / feat-059 (OpenCode) are independent and unblock more runtimes. Also: feat-035 (config env vars, phase-5 orphan) and feat-060 (attended mode, deferred by design). Phase-12 is now complete.
- **Build status:** green — `./init.sh` 3-layer gate passes (891 Rust + 142 frontend tests, clippy + fmt + prettier + ESLint clean, server starts, smoke test green, real `claude --version` health-check succeeds in ~80ms)
- **Test status:** green — 891 Rust + 142 frontend (was 871 + 142; +20 net from feat-067: 11 watch store + 7 lifecycle + 2 SSE bump). 18 named conformance tests still pass.
- **Lint status:** green — clippy clean on all targets (lib + bin + tests); no new warnings.
- **Precommit hook:** active (`core.hooksPath = .githooks`).

## Next Steps (in order)

1. **Pick the next `not_started` feature** — Phase-12 has 1 more: feat-067 (lifecycle supervision — needs feat-066 ✓). The Phase-10 follow-ups feat-058 (Codex) and feat-059 (OpenCode) are independent and unblock more runtimes. Also available: feat-035 (config env vars, phase-5 orphan) and feat-060 (attended mode, deferred by design). Per WIP=1, exactly one feature in `active` state.
