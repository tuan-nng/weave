...
- **Latest commit:** not committed yet (feat-029 A2A implementation complete)
- **Active feature:** none — feat-029 (A2A server endpoints) fully implemented; verification command `cargo test -p weave-server -- test_a2a` exits 0 (13/13 passing); `./init.sh` all 3 layers pass
- **Build status:** green — `cargo build -p weave-server` succeeds; `./init.sh` all 3 layers pass
- **Test status:** green — 582 Rust tests (was 569, +13 A2A: 8 types + 5 auth) + 76 frontend tests pass
- **Lint status:** green — clippy clean, fmt clean, prettier clean, ESLint clean
- **This session**: feat-029 A2A server endpoints implemented. Replaced the original feat-029 (inter-agent communication tools) with Google A2A protocol v1.0 REST binding. Core subset: Agent Card (GET /.well-known/agent.json), SendMessage (POST /api/a2a/messages), GetTask (GET /api/a2a/tasks/{id}), CancelTask (POST /api/a2a/tasks/{id}/cancel), SubscribeToTask SSE (GET /api/a2a/tasks/{id}/subscribe). All 3 layers green.
...
- [x] **feat-027**: Default board template + built-in specialists. 5 specialist markdown files created in `resources/specialists/` with YAML frontmatter (name, description, tool_profile, tags) + system prompt body adapted from routa's kanban specialist definitions. Each specialist has entry/exit gates forming a coherent pipeline: backlog-refiner (planning, turns rough cards into stories with acceptance criteria) → todo-orchestrator (planning, validates stories, adds execution plans and risk notes) → dev-crafter (implementation, implements changes, runs verification, commits) → review-guard (review, independently verifies acceptance criteria, rejects/approves) → done-reporter (reporting, writes completion summaries). `crates/weave-server/src/store/boards.rs` extended with 2 tests: `test_default_board_template` (creates board with 5-column template, verifies column names, specialist_ids, auto_trigger flags, ascending positions, board name) and `test_default_board_specialists_load_from_disk` (loads all 5 from `resources/specialists/` via `SpecialistRegistry::load_from_dir`, verifies name, tool_profile, description prefix, non-empty system_prompt, kanban tag). Startup log confirms `Specialists loaded loaded=5 skipped=0`. 397 Rust tests (was 395, +2) + 76 frontend tests; clippy + rustfmt + prettier clean. All 3 layers pass. Phase 6 review: 0 critical/high issues; 2 minor fixes applied (board name assertion, tags assertion). 5 files created, 1 modified.
- [x] **feat-028**: Kanban tools for agents. Four tool executors in `crates/weave-server/src/tools/kanban/`: `get_board` (composite board state), `create_card` (insert task on column with title/description/status validation), `search_cards` (free-text query against title+description + board_id/column_id/status filters), `move_card` (cross-column move with transition gates). All four hold `Arc<Db>`, validate input via shared helpers in `tools/fs/mod.rs`, and run workspace-scoped queries. Three transition gates implemented in `service::kanban::check_transition_gates`: (1) description frozen on exit (`column.freeze_description`), (2) required fields on entry (per-name lookup against task.acceptance_criteria/completion_summary/verification_report), (3) required artifact types (STUB for feat-031 — column has the field but the artifacts table doesn't exist yet). Same-column moves short-circuit, all-defaults columns take a fast-path. `move_card` is intentionally a thin wrapper: it resolves task + columns, runs the gate, delegates to `TaskStore::move_to_column`, optionally updates status. It does NOT fire `try_automate_lane` or SSE — the HTTP PATCH path remains the authoritative event source until runtime-dispatch lands. All 4 tools registered on the `planning` tool profile in `main.rs`. Migration 006 adds 3 columns to `columns`: `freeze_description INTEGER`, `required_fields TEXT` (JSON array), `required_artifact_types TEXT` (JSON array) — JSON-array form (not relational) chosen for forward-compat with feat-031. `store::columns::Column` and `store::boards::NewColumnSpec` and the create/update column API endpoints gain 3 new fields. `store::tasks::TaskStore::list` gains a `query: Option<&str>` parameter for free-text search. `store::tasks::validate_status` is the canonical validator (moved out of the task tools to be shared). `tools::fs::mod` gets `check_optional_status` + `validate_task_title` + `MAX_TASK_TITLE_LEN` so tool-level input validation matches the HTTP API. 441 Rust tests (was 397, +44) + 76 frontend tests, all green; clippy + rustfmt + prettier + ESLint clean. All 3 layers pass. Verification command `cargo test -p weave-server -- test_kanban_tools && cargo test -p weave-server -- test_move_card_transition_gates` exits 0 (2/2 matching tests). 5 files new (1 migration, 4 tool files) + 1 new tools/kanban/mod.rs + 9 files modified. **Phase 5b /simplify applied**: replaced `KNOWN_REQUIRED_FIELDS` const + parallel match with `task_field_value()` helper; moved same-column gate-skip into `check_transition_gates` itself (so future callers don't have to special-case it); added all-defaults fast-path; added no-op status update skip in `move_card` (avoids redundant UPDATE + `updated_at` bump when `s == moved.status`); made `is_blank` `pub(crate)` so the gate module can use it; extracted `validate_task_title` + `MAX_TASK_TITLE_LEN` to `tools::fs` to deduplicate with HTTP layer; removed dead `MAX_TITLE_LEN` constant in `create_card.rs`; fixed test fixture collision where `default_column` and `gate_column` both returned `id="col-test"` — same-column short-circuit was masking 5 transition-gate tests; added `id` parameter to both fixtures.
- [x] **feat-029**: A2A (Agent-to-Agent) protocol server endpoints. Replaces the original feat-029 spec (inter-agent communication tools) with Google A2A v1.0 REST binding core subset. New `src/a2a/` module with 6 files: `types.rs` (A2A protocol types — AgentCard, Task, TaskStatus with `from_session_status` session→A2A state mapper, SendMessageRequest, Part enum with serde-tagged Text/File/Data, TaskEvent for SSE, `extract_text_from_parts` helper), `auth.rs` (Bearer token verification against `WEAVE_A2A_TOKEN` env var; auth disabled when not configured for localhost dev), `agent_card.rs` (GET `/.well-known/agent.json` handler — builds dynamic AgentCard from loaded specialists as skills, Host header for URL, CARGO_PKG_VERSION), `messages.rs` (POST `/api/a2a/messages` — SendMessage: creates or continues Weave sessions, extracts text from incoming message parts, sends prompt via SessionService, returns Task), `tasks.rs` (GET `/api/a2a/tasks/{id}` — GetTask with optional history + artifacts; POST `/api/a2a/tasks/{id}/cancel` — CancelTask, idempotent; GET `/api/a2a/tasks/{id}/subscribe` — SubscribeToTask SSE stream using `futures::stream::unfold` with `SubscribeState` enum, emitting `TaskStatusUpdate` events, closing on terminal status), `mod.rs` (submodule declarations + Axum router). Migration 009 adds `context_id TEXT` column + index to `sessions` for multi-session A2A task linking. `AppState` gains `pub a2a_token: Option<String>` field. `AppError` gains `Unauthorized(String)` variant (401 mapping). `Session::context_id` field added with all indices in `map_row`, INSERT, RETURNING updated. `SessionStore::create` and `create_tx` gain `context_id: Option<&str>` parameter (8th positional). `SessionService::create_session` gains `context_id: Option<&str>` parameter (passes through to `create_tx`). `WorkspaceStore::get_default_id` helper returns default workspace UUID for A2A message routing. `api/mod.rs` registers `/.well-known/agent.json` route and nests A2A router at `/api/a2a`. A2A task ID = Weave session UUID; sessions ARE the A2A targets (no new entity table). 582 Rust tests (was 569, +13 A2A: 8 types + 5 auth) + 76 frontend tests, all green; clippy + rustfmt + prettier + ESLint clean. All 3 layers of `./init.sh` pass. Verification command `cargo test -p weave-server -- test_a2a` exits 0 (13/13 passing). 6 new files + 9 modified files. Total: 15 files.

- [x] **feat-031**: Artifact tools for agents. Replaces the feat-028 gate-3 STUB with a real `(task_id, type)` evidence store and 3 tool executors: `request_artifact` (create row, Conflict on duplicate), `provide_artifact` (single-statement upsert via `INSERT ... ON CONFLICT(task_id, type) DO UPDATE` — preserves `id` and `created_at`, bumps `updated_at`), `list_artifacts` (workspace-scoped list with optional `type` filter). New `artifacts` table (migration 007) with `UNIQUE(task_id, type)` index, `task_id ... ON DELETE CASCADE` FK, all `IF NOT EXISTS` for idempotency. New `store/artifacts.rs` (`ArtifactStore` with 5 methods: `create`, `provide`, `get_by_id`, `list_by_task`, `list_types_for_task`; `verify_task_in_workspace` helper for cross-workspace defense; `map_insert_error` to map `ConstraintViolation` → `AppError::Conflict`). `service::kanban::check_transition_gates` gained a 4th arg `present_artifact_types: &HashSet<String>` (pre-loaded by the caller — keeps the function pure) and the real gate-3 body. `move_card` pre-loads the set via `ArtifactStore::list_types_for_task` before invoking the gate. `Artifact` struct uses `#[serde(rename = "type")]` on the `type_` field (Rust keyword workaround). 491 Rust tests (was 441, +50), all green; clippy + rustfmt clean; all 3 layers of `./init.sh` pass. Verification command `cargo test -p weave-server -- test_artifact_crud && cargo test -p weave-server -- test_artifact_transition_gate` exits 0 (2/2 passing). 5 new files (1 migration, 1 store, 3 tool files, 1 tool mod.rs) + 7 modified. **Phase 6 quality review complete** (3 parallel agents — Simplicity, Correctness, Conventions). Findings: 1 critical (duplicate `seed_artifact_row` helper in `store/artifacts.rs` — inner copy inside `mod tests` is dead code; delete the inner copy to fix), 7 important (`map_insert_error` overclassifies FK/NOT-NULL as Conflict — narrow to `extended_code == 2067` per `workspaces.rs` precedent; `move_card` description doesn't mention the new artifact gate; `let _ = ws;` leftover in `list_artifacts.rs:191-192`; cross-workspace test divergence across the 3 tool files; `test_artifact_transition_gate` docstring claims re-blocking coverage the test doesn't deliver; phantom `task_field_value` reference in `move_card.rs:514` comment; `test_provide_updates_when_present_preserves_id` doesn't assert `updated_at` is bumped), 6 nice-to-have (`get_by_id` is dead code; `verify_task_in_workspace` duplicates 2 other workspace-lookup helpers pre-existing in the repo; `seed_task` helper duplicated 5× across artifact files; extra DB hit on `move_card` when no `required_artifact_types`; TOCTOU between load and gate is silent; `request_artifact` description encourages Conflict-prone usage). User decision pending.
- [x] **feat-030**: Note tools for agents. Five tool executors in `crates/weave-server/src/tools/note/`: `create_note` (title + type + content, Conflict on duplicate title within a workspace), `read_note` (workspace-scoped fetch by id), `list_notes` (workspace-scoped list with optional `type` filter, ordered `updated_at DESC, id DESC`), `set_note_content` (replace in full, bumps `updated_at`), `append_to_note` (single-statement `UPDATE ... SET content = content || ?2` atomic concat). New `notes` table (migration 008) with `UNIQUE(workspace_id, title)` index (the title-scope uniqueness the spec requires), 2nd `idx_notes_workspace` index for list path planner predictability, `workspace_id ... ON DELETE CASCADE` FK, all `IF NOT EXISTS` for migration idempotency. New `store/notes.rs` (`Note` struct with `#[serde(rename = "type")]`, stateless `NoteStore` with 5 methods: `create`, `get_by_id`, `list`, `set_content`, `append`; `validate_note_type` enforces the strict whitelist `["spec", "task", "general"]`; `validate_note_title` trims whitespace, rejects empty-after-trim, caps at `MAX_NOTE_TITLE_LEN = 200` chars). **3rd-caller `map_insert_error` hoist**: the helper moved from per-store private fns to `crate::db::map_insert_error(e, &str conflict_message, &str fk_resource)` so artifacts, codebases, and notes all share one canonical implementation (deferred item from feat-032 finally triggered). Asymmetric tool profile distribution per the user-confirmed design: read-only tools (`read_note`, `list_notes`) are in `full` + `implementation` + `review` + `planning` + `reporting` profiles; write tools (`create_note`, `set_note_content`, `append_to_note`) are in `full` + `implementation` only. `service::sessions::test_tool_profile_filtering` updated to replace the placeholder `MockTool::new("notes")` with the 2 real tool names and expect 10 names in the planning profile assertion (was 8). 569 Rust tests (was 518, +51), all green; clippy + rustfmt clean; all 3 layers of `./init.sh` pass. Verification command `cargo test -p weave-server -- test_note_crud && cargo test -p weave-server -- test_note_append` exits 0 (2/2 passing). 7 new files (1 migration, 1 store, 5 tool files + 1 tool mod.rs = 6 tool files) + 6 modified. **Phase 5b /simplify applied inline**: chose single-statement `UPDATE ... SET content = content || ?2` (no read-then-write), chose strict type whitelist over free-vocabulary (design decision), dropped no-op short-circuit on `set_content` (always UPDATE, matches `artifact::provide` precedent). Phase 6 + Phase 7 are reserved for the next session per the user-approved workflow.

## In Progress

(none — feat-032 complete; feat-029 A2A just completed; ready for next feature)
...
From feat-028 code review (3 parallel agents), deferred for follow-up:
- **#1 (deferred — user-chosen scope)**: HTTP PATCH `update_task` at `api/kanban.rs:382-456` calls `TaskStore::move_to_column` directly without invoking `check_transition_gates`. The frontend drag-and-drop bypasses the gate. The user explicitly chose "tools only" scope in feat-028's clarifying questions, so this is out of scope for this feature. Fix for a follow-up: either call `check_transition_gates` from `update_task`, or push the gate check into `TaskStore::move_to_column_tx` so all callers are covered (closes both this and #2). Documented in the move_card tool's module-level docstring.
- **#2 (deferred — pairs with #1)**: TOCTOU between `check_transition_gates` (read tx) and `TaskStore::move_to_column` (write tx). SQLite WAL serializes writers so the window is tight, but the gate's correctness is only as tight as the window between the two transactions. Fix is the same as #1: push the check into the move transaction.
- **#3 (deferred — pre-existing)**: `MAX_TASK_TITLE_LEN` defined in two places — `tools/fs/mod.rs:296` and `api/kanban.rs:41` — synced by comment. Fix: hoist `validate_task_title` + `MAX_TASK_TITLE_LEN` into `store::tasks` and have both layers import the canonical version. Pre-existing duplication; the tools-only scope for feat-028 only added the second copy.
...
- **#12 nice-to-have (defer)**: `move_card` calls `ArtifactStore::list_types_for_task` unconditionally before the gate, even when the destination has no `required_artifact_types` (the common case). Fix: skip the load with `if dest_column.required_artifact_types.is_empty() { HashSet::new() } else { ... }`. Saves one JOIN per move in the common path.
- **#13 nice-to-have (defer)**: TOCTOU between `list_types_for_task` and `move_to_column` is silent — a concurrent `provide_artifact` between the two calls would let a move succeed on freshly-added evidence. Acceptable per the tool-call threat model ("your move is authorized by the artifacts you had when you asked"), but worth a one-line comment noting the gate is a snapshot, not a held lock.
- **#14 nice-to-have (defer)**: `request_artifact` description encourages an awkward usage pattern: most agents will probably just use `provide_artifact` (which upserts) for everything, so `request_artifact` is the "I'll fill it in later" path but the description encourages also using it as a normal create — both paths a user would reasonably take from the description trip the Conflict. Worth a UX pass on the tool description or a design conversation about whether `request_artifact` should also be an upsert.

## Next Steps

1. feat-030 fully closed out — all 7 phases complete (Discovery → Codebase Exploration → Clarifying Questions → Architecture Design → Implementation → Quality Review → Summary). All 10 build-sequence steps green, verification command passes, `./init.sh` all 3 layers pass. Phase 6 (3 parallel code-reviewer agents) found 0 critical issues; 2 minor fixes applied (test names + doc comment).
2. Pick the next `not_started` feature from `feature_list.json`. The remaining candidates with all-passing dependencies are feat-029, feat-033, feat-034, feat-035. feat-033 (Enhanced health check) is the simplest +1-week starter; feat-029 (Inter-agent communication) is the natural follow-on to feat-028 + the kanban spec rows.

## Session Notes

### 2026-06-03 — feat-030 (Note tools) implementation + 3rd-caller `map_insert_error` hoist

Full 7-phase workflow. Chose feat-030 next (the natural follow-on to feat-031/feat-032 — workspace-scoped text store, mirrors codebases structurally but parent-free). User-approved asymmetric profile design: read-only tools in 4 profiles, write tools in 2.

**Backend (Rust, +7 new, +6 modified):**
- `migrations/008_notes.sql` (NEW, 44 lines) — `notes` table with `UNIQUE(workspace_id, title)` (the spec's title-scope uniqueness), 2nd `idx_notes_workspace` for the list path's planner predictability, `workspace_id ... ON DELETE CASCADE` FK, all `IF NOT EXISTS` for idempotency
- `store/notes.rs` (NEW, ~500 lines) — `Note` struct with `#[serde(rename = "type")]` on `type_`; `NoteStore` with 5 methods (`create`/`get_by_id`/`list`/`set_content`/`append`); `validate_note_type` enforces the strict whitelist `["spec", "task", "general"]`; `validate_note_title` trims whitespace, rejects empty-after-trim, caps at `MAX_NOTE_TITLE_LEN = 200` chars; 24 unit tests (validate happy/reject, create all edge cases, get_by_id happy/unknown/cross-ws, list with-filter/cross-ws/order/cascade, set_content replace+cross-ws, append grow+cross-ws, workspace-deletion-cascades)
- `db.rs` (+44 lines net) — `MIGRATIONS` const +1 (`("008", ...)`); `map_insert_error` hoisted to `crate::db::map_insert_error(e, &str conflict_message, &str fk_resource)` — 3rd-caller trigger (artifacts, codebases, notes); `test_db_init` expected_tables += "notes", comment bumped to "all 13 tables"; `test_migrations_idempotent` user_version 7→8
- `store/{artifacts,codebases}.rs` (−66 lines combined) — local `map_insert_error` deleted, call sites use `crate::db::map_insert_error`; net −42 lines after the hoisted function
- `tools/note/{mod,create_note,read_note,list_notes,set_note_content,append_to_note}.rs` (NEW, ~860 lines total) — 5 tool executors mirroring `request_artifact` pattern (`pub struct XxxTool { pub db: Arc<Db> }`, 4 trait methods, 4-7 unit tests each); 2 verification-gate tests in `mod.rs::tests` (`test_note_crud` end-to-end across all 5 tools + cross-workspace rejection; `test_note_append` focused 3-append linear-growth + `updated_at` bumps with 1.1s sleeps for rfc3339 second-precision)
- `tools/mod.rs` (+12/−2) — `pub mod note;`; 4 profiles updated asymmetrically: `implementation` gets all 5; `review`/`reporting` get `read_note` + `list_notes` only; `planning` replaces the pre-registered placeholder `"notes"` with the 2 real tool names
- `main.rs` (+12 lines) — 5 `tool_registry.register(Arc::new(tools::note::XxxTool { db: db.clone() }))` calls
- `service/sessions.rs` (+4/−2) — `MockTool::new("notes")` placeholder at line 2132 replaced with `read_note` + `list_notes`; `test_tool_profile_filtering` assertion updated to expect 10 names (was 8), sorted alphabetically

569 Rust tests (was 518, +51) + 83 frontend tests, all green. clippy clean (`cargo clippy -p weave-server -- -D warnings`, the same invocation as `just lint`), rustfmt + prettier + ESLint clean. `./init.sh` all 3 layers pass (clippy + 569 Rust + 83 frontend + smoke test for `/api/health` and `GET /`). Migration 008 confirmed on a real DB (startup log: `Migration applied version=8`).

**Out-of-Scope Items Noticed (logged for follow-up):**
- 1 pre-existing `type_complexity` warning at `service/sessions.rs:869` (unchanged from before this session, also flagged in prior sessions; not feat-030-introduced).
- The same "SessionStart hook additional context" session-end checklist rule applies: this session left the working tree green, but the diff is not yet committed — the user commits at their discretion per the prior-session pattern.

### 2026-06-03 — feat-030 Phase 6 (Quality Review) + Phase 7 (Summary)

3 parallel `feature-dev:code-reviewer` agents reviewed the full feat-030 footprint (7 new + 6 modified files, ~1,300 new lines):

| Focus | Agent | Verdict |
|-------|-------|---------|
| Simplicity & DRY | `abe92a9` | Pass — no issues ≥80 confidence |
| Correctness | `a9fcd40` | Pass — all 10 checks green (SQL injection, workspace scoping, cross-ws defense, thread safety, TOCTOU, error mapping, migration, profiles, tool registration); 0 critical/high issues |
| Conventions | `a24af32` | Pass with fixes — 2 issues found |

**2 fixes applied (this session):**
1. `store/notes.rs:377,389,470` — 3 test functions renamed `_returns_bad_request` → `_returns_validation`. `AppError::BadRequest` doesn't exist; the correct variant is `AppError::Validation`. The test bodies were already correct (matching `Validation(msg)`), only the names were misleading.
2. `db.rs:188` — doc comment "Verify all 12 tables exist" → "Verify all 13 tables exist" (notes table added in feat-030).

All 28 note tests (`cargo test -p weave-server -- notes`) still pass. `./init.sh` all 3 layers green. Phase 7 summary presented; all 7 feature-dev workflow phases complete.
...
1. feat-030 fully closed out — all 10 build-sequence steps green, verification command passes, `./init.sh` all 3 layers pass. Phase 6 (3 parallel code-reviewer agents) and Phase 7 (summary) are next; user-facing summary will be presented at session end.
2. Pick the next `not_started` feature from `feature_list.json`. The remaining candidates with all-passing dependencies are feat-029, feat-033, feat-034, feat-035. feat-033 (Enhanced health check) is the simplest +1-week starter; feat-029 (WorktreeStore) is the natural follow-on to feat-032 + the kanban spec rows.
