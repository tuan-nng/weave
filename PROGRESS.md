...
- **Latest commit:** **fix-070** (e66633a) — kind-aware Add Provider modal 400'd on `type: "claude-code"` for CLI rows. Widened the `provider_type` allowlist in `api/providers.rs:82-90` to accept both `"anthropic"` (HTTP) and `"claude-code"` (CLI), updated the now-stale "current `provider_type` discriminator is `"anthropic"`" comment in `agent/registry.rs::create_cli_agent_from_row`, and added regression test `test_create_cli_provider_accepts_type_claude_code`. 804 Rust + 142 frontend tests pass; `./init.sh` 3-layer gate green. Full journal: `PROGRESS-archive.md` § fix-070. **Previous commit:** **feat-057** (shared CLI adapter conformance suite, 18 conformance tests across 7 cases).
