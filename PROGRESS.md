...
- **Latest commit:** **feat-066** — Full transition-gate enforcement. New `AutomationConfig` struct (delivery/contract/checklist/validator rules + `gate_mode` toggle) on `columns.automation_json` via migration 016. `check_transition_gates` is now async, takes `Option<&GateContext>`, evaluates 4 new gate types with per-column `gate_mode` (blocking → reject, warning → log + allow). New `kanban_validations` table caches validator subprocess results by `(task_id, command_key)`. 871 Rust + 142 frontend tests pass; `./init.sh` 3-layer gate green.
- **Active feature:** none — feat-066 is `passing` in `feature_list.json` with evidence. Per WIP=1, pick the next `not_started` feature. Phase-12 remaining: feat-067 (lifecycle supervision — needs feat-066 ✓). Alternatively, Phase-10 follow-ups feat-058 (Codex) / feat-059 (OpenCode) are independent and unblock more runtimes. Also: feat-035 (config env vars, phase-5 orphan) and feat-060 (attended mode, deferred by design).
- **Build status:** green — `./init.sh` 3-layer gate passes (871 Rust + 142 frontend tests, clippy + fmt + prettier + ESLint clean, server starts, smoke test green, real `claude --version` health-check succeeds in ~80ms)
- **Test status:** green — 871 Rust + 142 frontend (was 857 + 142; +14 net from feat-066: 13 new gate tests + 1 worktree-isolation test). 18 named conformance tests still pass.
