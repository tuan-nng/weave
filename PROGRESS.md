...
- **Latest commit:** **fix-072** — Wrapped (`mode="wrapped"`) Claude Code sessions exited with status `error` within ~200ms and never produced an assistant turn. Root cause in `ClaudeCodeCodingAgent::build_args` (`crates/weave-server/src/agent/claude_code/agent.rs:212`): the real `claude` CLI does not accept `--prompt` (it rejects with `error: unknown option '--prompt'`), but `build_args` pushed `--prompt <text>` per the v1 argv-mode design. Replaced with `--print <text>` (enables non-interactive mode) + the prompt as a positional arg, plus `--verbose --output-format stream-json` (required for the real CLI to emit `{"type":"assistant",...}` / `{"type":"result",...}` events the parser consumes — without these the response is plain text the parser drops). Updated `ClaudeCodeStreamParser` (`crates/weave-server/src/agent/claude_code/parser.rs`) with `handle_real_assistant` (flattens Anthropic-API-shaped `message.content[]` blocks into TextDelta/Thinking/ToolUseStart) and `handle_real_result` (maps `is_error=true` to Error, `is_error=false` to Done with `stop_reason`). 4 files changed (+373 −6): `agent.rs` (build_args), `parser.rs` (2 new handlers + doc rewrite), `parser_test.rs` (4 new real-CLI tests: `real_cli_assistant_text`, `real_cli_assistant_thinking`, `real_cli_result_success`, `real_cli_result_error`), `PROGRESS.md`. 828 Rust + 142 frontend tests pass; `./init.sh` 3-layer gate green. End-to-end verified via real `claude` CLI: fresh wrapped session transitions `connecting`→`ready`, captures the CLI session id, and persists the assistant response. **Previous commit:** **fix-071** (4d95a73) — New-session wizard Step 1 "No healthy providers" cold-cache fix.
- **Active feature:** none — feat-057 is `passing` in `feature_list.json` with evidence. Per WIP=1, pick the next `not_started` feature (feat-058 Codex adapter or feat-059 OpenCode adapter, both depend on feat-057). **Phase-12 registered:** feat-063…feat-067 (Kanban × Claude Code Coordination, per `docs/design-docs/kanban-claude-code-coordination.md`) added to `feature_list.json` as 5 sub-1000-line tasks; the design doc is now decomposed into an actionable WIP=1 queue. None of the new features are eligible to start until feat-051 + feat-055 (Rich Prompt) and feat-024 + feat-028 + feat-031 (Gates / Stage) are passing — those prerequisites are already passing today.
- **Build status:** green — `./init.sh` 3-layer gate passes (828 Rust + 142 frontend tests, clippy + fmt + prettier + ESLint clean, server starts, smoke test green, real `claude --version` health-check on `/home/novpla/.local/bin/claude` succeeds in ~80ms)
- **Test status:** green — 828 Rust + 142 frontend (was 824 + 142; +4 from this fix: 2 in `agent.rs` tests `test_build_args_prompt_uses_print_and_positional` + `test_build_args_omits_positional_prompt_when_no_user_message`, and 2 in `parser_test.rs` — the other 2 are updated version of the same). All 18 named conformance tests still pass.
...
3. **(Low priority, ask first) Reconcile the dirty `crates/weave-server/Cargo.toml` `default-run = "weave-server"` one-liner** — unrelated to fix-070; either commit it as a tiny chore (and confirm it doesn't trip the precommit hook's `just check`) or revert it. Currently left unstaged per "stay in scope".
