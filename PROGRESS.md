...
- **Pending commit (uncommitted, working tree):** **fix-070** — kind-aware Add Provider modal 400'd on `type: "claude-code"` for CLI rows. Widened the `provider_type` allowlist in `api/providers.rs:82-90` to accept both `"anthropic"` (HTTP) and `"claude-code"` (CLI), and updated the now-stale "current `provider_type` discriminator is `"anthropic"` for both HTTP and CLI rows" comment in `agent/registry.rs::create_cli_agent_from_row`. New regression test `test_create_cli_provider_accepts_type_claude_code` (804 Rust tests after the fix; 142 frontend unchanged). Live verified: the exact user payload (`binary_path: /home/novpla/.local/bin/claude`, `permission_mode: accept-edits`, `type: "claude-code"`) now returns 201 Created. Full journal: `PROGRESS-archive.md` § fix-070. **Not yet committed** — ask the user whether to commit as one `fix:` commit or split into `fix:` + `docs(registry):` + `docs(progress):`.
...
- **Build status:** green — `./init.sh` 3-layer gate passes (804 Rust + 142 frontend tests, clippy + fmt + prettier + ESLint clean, server starts, smoke test green, real `claude --version` health-check on `/home/novpla/.local/bin/claude` succeeds)
- **Test status:** green — 804 Rust + 142 frontend (was 803 + 142; +1 from `test_create_cli_provider_accepts_type_claude_code`). The 18 named conformance tests all pass.
- **Lint status:** green — clippy clean on all targets (lib + bin + tests); no new warnings.
- **Precommit hook:** active (`core.hooksPath = .githooks`).

## Next Steps (in order)

1. **Decide on fix-070 commit shape** — `fix(api): widen provider_type allowlist to accept claude-code for kind=cli rows` (1 commit) or split into `fix:` + `docs(registry):` + `docs(progress):` (3 commits). Per CLAUDE.md § Git Workflow, one commit per logical change is the default; the docs updates are tied to the same change so a single commit is cleanest.
2. **Pick the next `not_started` feature** — feat-058 (Codex adapter) or feat-059 (OpenCode adapter) are the natural successors; both depend on feat-057 (`passing`). Per WIP=1, exactly one feature in `active` state. The conformance suite is the forcing function — each adapter must pass all 18 tests.
...
4. **(Low priority, ask first) Reconcile the dirty `crates/weave-server/Cargo.toml` `default-run = "weave-server"` one-liner** — unrelated to feat-057; either commit it as a tiny chore (and confirm it doesn't trip the precommit hook's `just check`) or revert it. Currently left unstaged per "stay in scope".
