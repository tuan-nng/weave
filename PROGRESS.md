...
- **Latest commit:** **fix-071** — New-session wizard Step 1 reported "No healthy providers" on a cold cache. Added `state.registry.cached_health_summary().await` at the top of `list_providers` (`crates/weave-server/src/api/providers.rs:51-71`) so the per-id `cached_health_for` read sees a fresh snapshot. Replaced the misleading `test_provider_healthy_field_default_false_when_cache_cold` with `test_list_providers_reflects_failed_probe_as_unhealthy` (uses a stub unhealthy agent, no real network) and added `test_list_providers_warms_cache_on_cold_start` (regression test for this fix). 824 Rust + 142 frontend tests pass; `./init.sh` 3-layer gate green. Full journal: `PROGRESS-archive.md` § fix-071. **Previous commit:** **fix-070** (e66633a) — kind-aware Add Provider modal 400'd on `type: "claude-code"` for CLI rows.
...
- **Build status:** green — `./init.sh` 3-layer gate passes (824 Rust + 142 frontend tests, clippy + fmt + prettier + ESLint clean, server starts, smoke test green, real `claude --version` health-check on `/home/novpla/.local/bin/claude` succeeds in ~80ms)
- **Test status:** green — 824 Rust + 142 frontend (was 804 + 142; +20 from this fix: 18 conformance carry-over + 2 new from `test_list_providers_warms_cache_on_cold_start` and `test_list_providers_reflects_failed_probe_as_unhealthy`, -1 replaced misleading test). The 18 named conformance tests all pass.
- **Lint status:** green — clippy clean on all targets (lib + bin + tests); no new warnings introduced by this fix (37 pre-existing in `service/{kanban,sessions}.rs` and `cli_conformance.rs` from earlier work).
