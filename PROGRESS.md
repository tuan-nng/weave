...
- **Latest commit:** **feat-064** — Phase-12 step 2: kanban tool set. Two new tools (`update_card`, `update_task`) plus wiring of three existing tools (`list_artifacts`, `provide_artifact`, `create_note`) into the kanban integration test. New migration `014_task_extended_fields.sql` adds 5 nullable columns to `tasks`: `priority`, `labels`, `scope`, `verification_commands`, `test_cases`. `Task` and `UpdateTask` structs extended. `TaskStore::update`, `SELECT_COLS`, `RETURNING_COLS`, `map_row` updated. `api/kanban.rs` `UpdateTaskRequest` + `From` impl extended. 855 Rust + 142 frontend tests pass; `./init.sh` 3-layer gate green.
- **Active feature:** none — feat-064 is `passing` in `feature_list.json` with evidence. Per WIP=1, pick the next `not_started` feature. Phase-12 queue: feat-065 (column.stage enum + automation_json), then feat-066 (delivery / contract rules), then feat-067 (handoff blob). Alternatively, Phase-9 follow-ups feat-058 (Codex) / feat-059 (OpenCode) are independent and unblock more runtimes.
- **Build status:** green — `./init.sh` 3-layer gate passes (855 Rust + 142 frontend tests, clippy + fmt + prettier + ESLint clean, server starts, smoke test green, real `claude --version` health-check succeeds in ~80ms)
- **Test status:** green — 855 Rust + 142 frontend (was 844 + 142; +11 net from feat-064: 6 new in update_card.rs + update_task.rs, 5 new integration steps in test_kanban_tools). 18 named conformance tests still pass.
- **Lint status:** green — clippy clean on all targets (lib + bin + tests); no new warnings.
- **Precommit hook:** active (`core.hooksPath = .githooks`).

## Next Steps (in order)

1. **Pick the next `not_started` feature** — Phase-12 has 3 more: feat-065 (column.stage enum + automation_json, migration 015), feat-066 (delivery / contract rules — unblocks the v1-omitted sections 5/7/8), feat-067 (handoff blob). The Phase-9 follow-ups feat-058 (Codex) and feat-059 (OpenCode) are independent and unblock more runtimes. Per WIP=1, exactly one feature in `active` state.
